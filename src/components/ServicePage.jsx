...
import {  MousePointer2Off , ChevronsRight } from "lucide-react"
  ...
  async function loadServices() {
    ...
    try {
      ...
      const normalized = (Array.isArray(items) ? items : []).map((s) => {
        ...
        // ✅ Supporte "disponible", "indisponible", "available", "unavailable", et boolean
        const available =
          typeof s.available === "boolean"
            ? s.available
            : typeof s.available === "string"
            ? ["available", "disponible"].includes(s.available.toLowerCase())
            : s.available === true;
